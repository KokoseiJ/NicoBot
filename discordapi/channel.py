...
class Channel(DictObject):
    ...
    def modify(self, postdata):
        ...
        channel_obj = self._send_request("PATCH", "", postdata)
    ...
    def get_messages(
        self, limit=EMPTY, around=EMPTY, before=EMPTY, after=EMPTY
    ):
        endpoint = "/messages?"
        postdata = {
            "around": around,
        ...
        messages = self._send_request("GET", endpoint)

        return [Message(self.client, message) for message in messages]

    def get_message(self, id_):
        message = self._send_request("GET", f"/messages/{id_}")
    ...
    def delete_message(self, message):
        ...
        self._send_request("DELETE", f"/messages/{message}")
    ...
    def delete_messages(self, messages):
        ...
        self._send_request("POST", "/messages/bulk-delete", postdata)

    def typing(self):
        self._send_request("POST", "/typing")

    def get_pinned_messages(self):
        messages = self._send_request("GET", "/pins")
    ...
    def pin_message(self, message):
        ...
        self._send_request("PUT", f"/pins/{message}")

    def unpin_message(self, message):
        if isinstance(message, Message):
            message = message.id

        self._send_request("DELETE", f"/pins/{message}")
    ...
    def react(self, message, emoji, urlencoded=False):
        ...
        self._send_request("PUT", f"/messages/{message}/reactions/{emoji}/@me")
    ...
    def delete_my_reaction(self, message, emoji, urlencoded=False):
        ...
        self._send_request(
            "DELETE", f"/messages/{message}/reactions/{emoji}/@me"
        )
    ...
    def delete_others_reaction(self, message, emoji, user, urlencoded=False):
        ...
        self._send_request(
            "DELETE", f"/messages/{message}/reactions/{emoji}/{user}"
        )

    def get_reactions(
        self, message, emoji, limit=EMPTY, after=EMPTY, urlencoded=False
    ):
        if isinstance(message, Message):
            message = message.id
        if not urlencoded:
        ...
        postdata = {"after": after, "limit": limit}
        postdata = clear_postdata(postdata)
        ...
        users = self._send_request("GET", endpoint)
    ...
    def delete_all_reactions(self, message):
        ...
        self._send_request("DELETE", f"/messages/{message}/reactions")
    ...
    def delete_all_reactions_for_emoji(self, message, emoji, urlencoded=False):
        ...
        self._send_request("DELETE", f"/messages/{message}/reactions/{emoji}")
...
class GroupDMChannel(DMChannel):
    def modify(self, name=EMPTY, icon=None):
        ...
        postdata = json.dumps({"name": name, "icon": icon})
...
class GuildChannel(Channel):
    ...
    def edit_permission(self, id_, allow=EMPTY, deny=EMPTY, type=EMPTY):
        postdata = {"allow": allow, "deny": deny, "type": type}
        postdata = clear_postdata(postdata)

        self._send_request("PUT", f"/permissions/{id_}", postdata)

    def remove_permission(self, id_):
        self._send_request("DELETE", f"/permissions/{id_}")

    def get_invites(self):
        invites = self._send_request("POST", "/invites")
        return invites
    ...
    def invite(
        ...
        postdata = clear_postdata(postdata)
        invite = self._send_request("POST", "/invites", postdata)
        return invite
...
class GuildTextChannel(GuildChannel):
    def modify(
        ...
        topic=EMPTY,
        nsfw=EMPTY,
        rate_limit_per_user=EMPTY,
        permission_overwrites=EMPTY,
        parent_id=EMPTY,
    ):
        postdata = {
            "name": name,
    ...
    def crosspost(self, message):
        if isinstance(message, Message):
            message = message.id
        rtnmsg = self._send_request("POST", f"/messages/{message}/crosspost")
        return Message(self.client, rtnmsg)

    def follow_news_channel(self, id_):
        postdata = {"webhook_channel_id": id_}
        return self._send_request("POST", "/followers", postdata)
